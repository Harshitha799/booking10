...
public class BookingServiceImpl implements IBookingService
{
	...
	public long addBooking(BookingDto bookingdto)
	{
		...
		Passenger10 passenger = passengerRepository.findById(bookingdto.getPassengerId()).get();
		booking.setPassengerInfo(passenger);
		User10 user = userRepository.findById(bookingdto.getUserId()).get();
	...
	@Override
	public void updateBookingById(long bookingId, BookingDto bookingdto) 
	{
		
		if(bookingRepository.existsById(bookingId)) 
		{
			Booking10 updatebooking = bookingRepository.findById(bookingId).get();
			updatebooking.setDestination(bookingdto.getDestination());
			updatebooking.setBusNumber(bookingdto.getBusNumber());
